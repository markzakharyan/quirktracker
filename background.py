...
def process(passed: list) -> None:
    ...
    data = [] #[[0] * 11 for _ in range((len(DD) - 1) * 16)]
    # count = 1
    data[0] = ["EventID", "TruthID", "PID", "Layer", "r[cm]", "phi", "z[cm]", "E[GeV]", "px[GeV]", "py[GeV]", "pz[GeV]"]
    oopsies=0
    ...
    path = os.path.join(path, "Bgd_500_1jet_wCuts_nozeros.csv")
    df = pd.DataFrame(data[1:], columns=data[0])
    df.to_csv(path, index=False)

if __name__ == "__main__":
    # raise Exception("This file is not meant to be run directly. Please import it from another file.")
    # Old default set of events below

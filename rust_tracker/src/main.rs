...
fn process(lambda: f64, input_path_p: &str, input_path_a: &str, output_path: &str) -> Vec<i32> {
        ...
        if aa.len() == 16 {
            ...
            for hit in aa {
                let record = vec![
                    ...
                    hit.0.to_string(),
                    hit.6.to_string(),
                    knappen::ATLAS_RADII_PIXEL[hit.0 as usize - 1].to_string(),
                    hit.2.to_string(),
                    hit.1.to_string(),
...
fn main() {
    ...
    println!("(Quirk Regular) Time taken: {:?} seconds", duration.as_secs_f32());
